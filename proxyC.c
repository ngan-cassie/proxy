...
int main(int argc, char **argv) {
    ...
    /*Moved here from print_log so it's not reset unexpectedly */
    ...
    while (1) {
        ...
        /*Moved inside of loop so each connection gets its own memory space*/
       ...
       pthread_create(&tid, NULL, thread, formargs); //thread to handle request
...
void *thread(void *vargp) {
    ...
    if((rio_readlineb(&rio, buf, MAXLINE)) == -1) {
        ...
        close(fd);
        free(formargs); //free to avoid memory leaks
        return (void *)0;
    }
    ...
    if(strcasecmp(method, "GET") != 0){
        ...
        close(fd); //close fd to avoid leaks
        free(formargs);//free to avoid memory leaks
        return (void *)0;
    }
